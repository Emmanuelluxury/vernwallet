        ...
        async function initiateBridgeTransfer() {
            ...
            try {
                ...
                // Validate form
                if (!fromAddress) {
                    showNotification('Please enter a source address', true);
                    fromAddressInput.focus();
                    return;
                }

                if (!toAddress) {
                    showNotification('Please enter a destination address', true);
                ...
                if (currentDirection === 'to-starknet') {
                    fromValid = validateBitcoinAddress(fromAddress);
                    toValid = validateStarknetAddress(toAddress);
                } else {
                    fromValid = validateStarknetAddress(fromAddress);
                    toValid = validateBitcoinAddress(toAddress);
                }
                ...
                if (!toValid) {
                    showNotification('Invalid destination address format', true);
                    toAddressInput.focus();
                    return;
                }
                ...
                // Update UI
                bridgeButton.disabled = true;
                bridgeButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processing...';
                cancelBridgeBtn.style.display = 'inline-flex';

                showNotification('Bridge transfer initiated! Processing...', false);

                // Start progress simulation
                startBridgeProgress();

                console.log('✅ Bridge transfer validation passed');
                ...
                console.error('❌ Bridge transfer initiation failed:', error);
                showNotification(`Bridge transfer failed: ${error.message}`, true);
        ...
        function cancelCurrentTransaction() {
            if (currentTransaction.isActive) {
                ...
                showNotification('Transaction cancelled', false);
            }
        }

        // Start bridge progress simulation
        function startBridgeProgress() {
            let progress = 0;
            const totalDuration = 15000; // 15 seconds for demo
        ...
        function completeBridgeTransaction() {
            ...
            // Show success notification
            showNotification(`✅ Bridge transaction completed! ${currentTransaction.amount} BTC transferred successfully.`, false);
        ...
        async function copyTransactionHash(element) {
            try {
                const hash = element.textContent || element.innerText;
                ...
                showNotification('Failed to copy transaction hash', true);
            }
        }
        ...
        async function stakeTokens(amount) {
            ...
            try {
                if (!amount || parseFloat(amount) <= 0) {
                    showNotification('Please enter a valid amount to stake', true);
                    return;
                }

                // Check if wallet is connected
                if (!connectedAddresses.starknet) {
                    showNotification('Please connect your Starknet wallet first', true);
                    return;
                }

                showNotification(`Staking ${amount} BTC... (Demo - not implemented)`, false);

                // Update staking data (mock)
                bridgeState.stakingData.stakedAmount += parseFloat(amount);
                updateStakingUI();

            } catch (error) {
                console.error('Staking failed:', error);
                showNotification(`Staking failed: ${error.message}`, true);
            }
        }
        ...
        async function unstakeTokens(amount) {
            ...
            try {
                if (!amount || parseFloat(amount) <= 0) {
                    showNotification('Please enter a valid amount to unstake', true);
                    return;
                }

                const unstakeAmount = parseFloat(amount);
                if (unstakeAmount > bridgeState.stakingData.stakedAmount) {
                    showNotification('Cannot unstake more than your staked amount', true);
                    return;
                }
                ...
                showNotification(`Unstaking ${amount} BTC... (Demo - not implemented)`, false);

                // Update staking data (mock)
                bridgeState.stakingData.stakedAmount -= unstakeAmount;
                updateStakingUI();

            } catch (error) {
                console.error('Unstaking failed:', error);
                showNotification(`Unstaking failed: ${error.message}`, true);
            }
        }
        ...
        async function claimStakingRewards() {
            ...
            try {
                ...
                showNotification(`Claiming ${bridgeState.stakingData.rewards.toFixed(6)} BTC rewards... (Demo - not implemented)`, false);

                // Update staking data (mock)
                bridgeState.stakingData.rewards = 0;
                updateStakingUI();

            } catch (error) {
                console.error('Claiming rewards failed:', error);
                showNotification(`Claiming rewards failed: ${error.message}`, true);
            }
        }
        ...
        // Get available wallets based on current direction
        function getAvailableWallets() {
            const wallets = [];

            if (currentDirection === 'to-starknet') {
                // Bitcoin to Starknet: Need Bitcoin wallet for source
                wallets.push({
                    id: 'xverse',
                    name: 'Xverse',
                    type: 'bitcoin',
                    isAvailable: function() {
                        return !!(window.XverseProviders?.BitcoinProvider ||
                                window.satsConnect ||
                                window.xverse ||
                                (window.ethereum?.providers && window.ethereum.providers.find(p =>
                                    p.isXverse === true || (p.constructor?.name?.includes('Xverse')))));
                    }
                });
            } else {
                // Starknet to Bitcoin: Need Bitcoin wallet for destination
                wallets.push({
                    id: 'xverse',
                    name: 'Xverse',
                    type: 'bitcoin',
                ...
                // Also need Starknet wallet for source
                wallets.push({
                    id: 'starknet',
                    name: 'Ready Wallet',
                    type: 'starknet',
        ...
        // Get wallet connection summary
        function getWalletConnectionSummary() {
            return {
                direction: currentDirection,
                bitcoin: {
                    connected: !!connectedWallets.bitcoin,
                    address: connectedAddresses.bitcoin,
                    wallet: connectedWallets.bitcoin
                },
                starknet: {
                    connected: !!connectedWallets.starknet,
                    address: connectedAddresses.starknet,
                    wallet: connectedWallets.starknet
                },
                bothConnected: !!(connectedAddresses.bitcoin && connectedAddresses.starknet),
                readyForBridge: checkWalletReadiness()
            };
        }
